...
function upload_about_image(string $fieldName, string $currentPath, array &$errors): string
{
    if (empty($_FILES[$fieldName]) || ! is_array($_FILES[$fieldName])) {
        return $currentPath;
    }
    ...
    if (! is_dir($directory) && ! mkdir($directory, 0775, true) && ! is_dir($directory)) {
        $errors[] = 'Unable to create the about-page upload directory.';
        return $currentPath;
    }
...
function upload_awards_image(string $fieldName, string $currentPath, array &$errors): string
{
    if (empty($_FILES[$fieldName]) || ! is_array($_FILES[$fieldName])) {
        return $currentPath;
    }
    ...
    $directory = PUBLIC_PATH . '/uploads/awards';

    if (! is_dir($directory) && ! mkdir($directory, 0775, true) && ! is_dir($directory)) {
        $errors[] = 'Unable to create the awards upload directory.';
        return $currentPath;
    }
